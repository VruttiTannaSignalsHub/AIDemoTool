...
class SignUpApp(HydraHeadApp):
    ...
    def _do_signup(self, form_data, msg_container) -> None:
        if form_data['submitted'] and (form_data['password'] != form_data['password2']):
            ...
            with st.spinner("🤓 now redirecting to login...."):
                ...
                time.sleep(2)
                form_data1=pd.DataFrame(form_data.items(), columns=['username', 'password'])
                form_data1.to_csv('data/login.csv')
    ...
    def _save_signup(self, signup_data):
        ...
        signup_data1=pd.DataFrame([*signup_data.values()])
        signup_data1=signup_data1.T
        mapping = {signup_data1.columns[0]: 'Username', signup_data1.columns[1]: 'password',signup_data1.columns[2]: 'confirm_password',signup_data1.columns[3]: 'Access_level',signup_data1.columns[4]: 'status'}
        su = signup_data1.rename(columns=mapping)
        #signup_data1=pd.DataFrame(data=signup_data1, columns=['firstcolumn','secondcolumn', 'thirdcolumn','fourthcolumn','fif'])
        su.to_csv('data/signup.csv')
